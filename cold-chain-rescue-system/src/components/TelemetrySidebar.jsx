  ...
  if (!selectedTruck) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-center text-slate-500">
        ...
        <p className="text-sm">Select a vehicle from the monitoring matrix to audit sensor telemetry.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-950 border border-slate-800 rounded-xl p-5 text-center shadow-inner">
        ...
        <p className="text-[10px] text-slate-500 mt-3 uppercase tracking-widest">120-Minute Lookahead Horizon</p>
      </div>

      <div className="space-y-3">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-4 border-b border-slate-800 pb-2">Live IoT Telemetry</h3>
        ...
        <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-4 flex justify-between items-center">
          ...
          <div className={`w-3 h-3 rounded-full ${selectedTruck.voltage < 12 ? 'bg-rose-500 animate-ping' : 'bg-emerald-500 shadow-[0_0_10px_#10b981]'}`} />
        </div>
        ...
        <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-4 flex justify-between items-center">
          ...
          <div className={`w-3 h-3 rounded-full ${selectedTruck.vibration > 70 ? 'bg-rose-500 animate-ping' : 'bg-emerald-500 shadow-[0_0_10px_#10b981]'}`} />
        </div>
      </div>
      ...
      {activeRescue && (activeRescue.failed_truck_id === selectedTruck.truck_id || activeRescue.rescue_truck_id === selectedTruck.truck_id) && (
        <div className="border border-indigo-500 bg-indigo-500/10 rounded-xl p-5 space-y-3 mt-8 shadow-[0_0_20px_rgba(99,102,241,0.2)]">
          <div className="text-xs font-bold uppercase tracking-wider text-indigo-400 flex items-center gap-2">
            ...
            Active Rescue Engaged
          </div>
          <div className="text-sm text-slate-200 leading-relaxed">
            <strong>{activeRescue.rescue_truck_id}</strong> routed to intercept payload from <strong>{activeRescue.failed_truck_id}</strong>.
          </div>
        </div>
      )}
    </div>
