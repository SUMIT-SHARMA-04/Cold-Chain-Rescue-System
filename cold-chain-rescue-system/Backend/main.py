...
try:
    # Updated to load your CatBoost model
    with open("cat_boost_model.pkl", "rb") as f:
        ml_brain = pickle.load(f)
    print("CatBoost AI Brain loaded successfully.")
except FileNotFoundError:
    print("WARNING: cat_boost_model.pkl not found. API will use hardcoded fallback logic.")
    ml_brain = None
...
def dispatch_nearest_rescue(failed_truck, fleet_list):
    """Finds the closest available healthy truck with sufficient cargo capacity."""
    closest_truck_id = None
    min_distance = float('inf')
    
    # Extract the payload volume that needs to be rescued
    failed_payload_volume = failed_truck.get("current_cargo_volume", 0)
    
    for truck in fleet_list:
        # Prevent the failing truck from trying to rescue itself
        if truck["truck_id"] == failed_truck["truck_id"]:
            continue
            
        # Calculate how much empty space the candidate truck has
        available_capacity = truck.get("max_capacity_volume", 100) - truck.get("current_cargo_volume", 0)
        
        # Candidate MUST be healthy AND have enough room for the payload
...
def process_telemetry(data: TelemetryData):
    ...
    if risk_prob > 80 and truck["fleet_status"] == "healthy":
        ...
        if rescue_truck_id:
            ...
            # Optional: Log if a critical failure occurs but no truck has enough space
            print(f"CRITICAL ALERT: {truck['truck_id']} failed. No fleet asset has sufficient capacity for rescue.")
